%% Operate
img_s = imread('air.png');
sz = size(img_s);
mask = imread('air2.png');
szm = size(mask);
if(length(szm)==3)
    if(szm(3)==3)
        mask = rgb2gray(mask);
...
while(any(mask(:)))
    ...
    overlap = 100000;
    patch_img = double(img_pnc(cur_patch(1):cur_patch(1)+2*max_ind,cur_patch(2):cur_patch(2)+2*max_ind,:));
